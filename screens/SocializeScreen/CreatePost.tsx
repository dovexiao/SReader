...
import { useCreatePostStore, ImageType } from '../../stores/createPost.store.ts';

const CreatePost: React.FC<NavigationProps> = ({ navigation }) => {
    const images = useCreatePostStore(state => state.postImages);
    const context = useCreatePostStore(state => state.postContext);
    const removeImage = useCreatePostStore(state => state.removeImage);
    const setPostImages = useCreatePostStore(state => state.setPostImages);
    const setPostContext = useCreatePostStore(state => state.setPostContext);
    ...
    const openImagePicker = async () => {
        try {
            ...
            if (result.assets) {
                ...
                const newImages: ImageType[] = result.assets.map(asset => ({
                    type: asset.type || '',
                    uri: asset.uri || '',
                    width: asset.width || 0,
            ...
            <ScrollView style={[styles.container, { backgroundColor: themes['background-basic-color-1'] }]}>
                <View style={styles.inputContainer}>
                    ...
                    <TextInput
                        ...
                        maxLength={500}
                    />
                    <Text style={styles.chartCount}>{context.length}/500</Text>
                </View>

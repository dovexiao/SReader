...
import SettingsAction from '@/center/recycleBin/components/questionRecycleBin/SettingsAction.tsx';
import { useQuestionRecycleBinStore } from '@/center/recycleBin/stores';

const ITEM_HEIGHT: number = 120;
const HEADER_HEIGHT: number = 40;

const buildGetItemLayout = getItemLayout<QuestionSection>({
...
const SectionBank = forwardRef<SectionBankAPI>((_, ref) => {
    const sectionBankRef = useRef<SectionList<QuestionSection>>(null);
    const themes = useTheme();

    const sections = useQuestionRecycleBinStore(state => state.sections);
    const setActiveLetter = useQuestionRecycleBinStore(state => state.setActiveLetter);
    ...
    // 渲染列表项
    const renderSection = ({ item }: { item: QuestionSection }) => (
        <View style={styles.itemContainer}>
            <View style={styles.header}>
                ...
                <TouchableOpacity onPress={() => {
                    bottomActionSheetRef.current?.show(<SettingsAction cardId={item.questionId}/>);
                }}>
    ...
    // 渲染分组标题
    const renderSectionHeader = ({ section }: { section: SectionListData<QuestionSection> }) => (
